...
Gua reproducible para levantar la aplicacin en una instancia Ubuntu 22.04 en AWS EC2.

## 1. Preparar la instancia
1. Crear instancia t3.small (o similar) con Ubuntu 22.04.
2. Security Group: abrir puertos **22** (SSH) y **80** (HTTP). HTTPS opcional segn el caso.
...
- Configuracin de correo y JWT segn necesidad.

> **Postgres**: usar RDS o una instancia propia. Si se usa RDS, exponer el SG solo al SG de la app (puerto 5432). Para Postgres local, instalar `postgresql` y crear base/usuario antes de migrar.

## 5. Migraciones y estticos
```bash
source venv/bin/activate
...
```
Asegrate de que la unidad apunte al binario `/opt/eva4/venv/bin/gunicorn` y a `config.wsgi:application`.
...
- Nginx sirviendo el dominio/IP pblico en el puerto 80.
- Migraciones aplicadas y usuario demo creado (si se ejecut `seed_demo`).
- Variables `.env` protegidas (permisos 600) y `DEBUG=False`.
- Evidencias a adjuntar por el usuario: IP pblica o Elastic IP asociada, y captura de pantalla de la app cargando va Nginx.
